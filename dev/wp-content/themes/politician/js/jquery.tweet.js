      ...
      $(widget).bind("load", function(){
        $.getJSON(build_url(), function(data){
          ...
          $.each(tweets, function(i,item){
            ...
            var date = '<div class="time"><a href="http://twitter.com/'+from_user+'/statuses/'+item.id+'" title="'+s.view_text+'">'+relative_time(item.created_at)+'</a></div>';
            var text = '<div class="tweet-content">' +$([item.text]).linkUrl().linkUser().linkHash().makeHeart().capAwesome().capEpic()[0]+ '</div>';
   
            // until we create a template option, arrange the items below to alter a tweet's display.
            list.append('<div class="tweet">' + avatar + join + text + date + '</div>');
